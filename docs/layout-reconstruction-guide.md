...
### 3.6 公式渲染（离线 LaTeX 子集）

测试台**不依赖任何外部库**（MathJax/KaTeX 均不引入），而是内置一个纯 JS 的轻量 LaTeX 渲染器（`renderLatex` + `_parseLatex`，位于 `static/index.html`）：

- **覆盖范围**：希腊字母（α β △ ∑ ∫ ∠ 等）、上下标 `^`/`_`、分式 `\frac{a}{b}`、根号 `\sqrt{x}`（含 `\sqrt[n]{x}`）、常见符号（`\leq \approx \times \cdot \circ \pm` 等）、`\text{}` 文本。
- **仅用于排版还原视图（两类公式都在此渲染，Markdown 视图保持 `$...$` 源码）**：
  1. **行内公式（真实数据的主要形态）**：Unlimited-OCR 不产出独立的 `formula` 元素（`latex` 字段恒为 `null`），公式一律以 `$...$` / `$$...$$` **内嵌在段落/标题/选项的 `content` 文本里**（如 `在 $\triangle ABC$ 中`）。前端用 `renderInlineLatex(text)` 把这些行内公式就地渲染成数学样式，非公式文字原样保留。
  2. **独立 `formula` 元素**：如 PaddleOCR-VL 返回 `type:"formula"` 且带 `latex`，走 `renderLatex()` 渲染其 `latex`/`content`；若二者皆空则回退显示该区域原图裁切或标注「（公式未识别）」。
  - Markdown 视图刻意保留原始 `$...$` 源码、不渲染——Markdown 预览就是引擎返回的原文，公式的数学样式统一交给排版还原视图。
- **容错**：无法识别的命令（如 `\left \right \begin`）直接跳过；解析异常时回退为原始文本，绝不炸页面。
- 渲染产物是普通 HTML（`sup`/`sub`/`frac`/`sqrt` 配合少量 CSS），零网络请求，离线可用。
